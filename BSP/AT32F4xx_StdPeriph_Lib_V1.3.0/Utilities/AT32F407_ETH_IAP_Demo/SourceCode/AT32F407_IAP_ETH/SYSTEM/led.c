...
void TMR3_GLOBAL_IRQHandler(void)
{
  ...
  {	
		TOGGLE_LED2(LED2_GPIO_PIN);
		if(GetDaraFromUSART_Flag)
		{
			...
			if(TIME_IRA_CNT>3)
				Back_Err();
			if(TIME_IRA_CNT>5)
				RCC_AHBPeriphClockCmd(LED2_GPIO_RCC, DISABLE);
		}
  ...
  {	
		...
		if(GetDaraFromUSART_Flag)
		{
			...
			if(TIME_IRA_CNT>3)
				Back_Err();
			if(TIME_IRA_CNT>5)
				RCC_APB2PeriphClockCmd(LED_GPIO_RCC, DISABLE);
		}
